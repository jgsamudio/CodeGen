...
private extension ProfileViewController {
    ...
    func setupDesign() {
        ...
        scrollView.autoPinEdgesToSuperviewEdges(with: UIEdgeInsets(top: navigationView.frame.maxY), excludingEdge: .bottom)
        scrollViewBottomConstraint = scrollView.autoPinEdge(.bottom, to: .bottom, of: view)
...
extension ProfileViewController: ProfileHeaderViewDelegate {
    ...
    func visit(_ socialLink: SocialLink) {
        if socialLink.isUniversalLink {
            UIApplication.shared.open(url: socialLink.url)
        } else {
            open(socialLink.url)
        }
    }
...
// MARK: - ProfileFooterItemDelegate
